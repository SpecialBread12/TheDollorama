...
namespace TheDollorama.Content.Items.Consumables
{
	...
	internal class DamageBoosterMagic : ModItem
	{
        ...
        {
            return player.GetModPlayer<DamageIncreasePlayerMagic>().damageBoostsMagic < MaxDamageBoosts;
        }

        public override bool? UseItem(Player player)
        {
            if (player.GetModPlayer<DamageIncreasePlayerMagic>().damageBoostsMagic >= MaxDamageBoosts)
            {
                return null;
            }

            // Augmente le compteur d'améliorations de dégâts du joueur
            player.GetModPlayer<DamageIncreasePlayerMagic>().damageBoostsMagic++;
