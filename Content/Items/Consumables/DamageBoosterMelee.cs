...
namespace TheDollorama.Content.Items.Consumables
{
	...
	internal class DamageBoosterMelee : ModItem
	{
        ...
        {
            return player.GetModPlayer<DamageIncreasePlayerMelee>().damageBoostsMelee < MaxDamageBoosts;
        }

        public override bool? UseItem(Player player)
        {
            if (player.GetModPlayer<DamageIncreasePlayerMelee>().damageBoostsMelee >= MaxDamageBoosts)
            {
                return null;
            }

            // Augmente le compteur d'améliorations de dégâts du joueur
            player.GetModPlayer<DamageIncreasePlayerMelee>().damageBoostsMelee++;
