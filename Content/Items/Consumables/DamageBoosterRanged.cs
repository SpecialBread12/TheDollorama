...
namespace TheDollorama.Content.Items.Consumables
{
	...
	internal class DamageBoosterRanged : ModItem
	{
        ...
        {
            return player.GetModPlayer<DamageIncreasePlayerRanged>().damageBoostsRanged < MaxDamageBoosts;
        }

        public override bool? UseItem(Player player)
        {
            if (player.GetModPlayer<DamageIncreasePlayerRanged>().damageBoostsRanged >= MaxDamageBoosts)
            {
                return null;
            }

            // Augmente le compteur d'améliorations de dégâts du joueur
            player.GetModPlayer<DamageIncreasePlayerRanged>().damageBoostsRanged++;
