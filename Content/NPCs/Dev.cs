	...
	public class Dev : ModNPC
	{
		...
		public override void SetStaticDefaults() {
            ...
            NPC.Happiness
                ...
                .SetNPCAffection(ModContent.NPCType<K>(), AffectionLevel.Hate)
                .SetNPCAffection(ModContent.NPCType<AssistantTeamChief>(), AffectionLevel.Love)
            ; // < Mind the semicolon!
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			...
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("He is the God of Dollorama, well at least in this realm."),

				// You can add multiple elements if you really wanted to
				// You can also use localization keys (see Localization/en-US.lang)
				new FlavorTextBestiaryInfoElement("He code badly, but it work :)")
			});
		...
		public override bool CanTownNPCSpawn(int numTownNPCs) { // Requirements for the town NPC to spawn.
			foreach (var player in Main.ActivePlayers) {
				...
				if (player.inventory.Any(item => item.type == ModContent.ItemType<CutPotion>())) {
					return true;
				}
			}
		...
		*/
		...
		public override void OnChatButtonClicked(bool firstButton, ref string shop) {
			if (firstButton) {
                ...
                if (Main.LocalPlayer.HasItem(ItemID.HiveBackpack)) {
					...
					Main.npcChatText = $"I upgraded your {Lang.GetItemNameValue(ItemID.HiveBackpack)} to a {Lang.GetItemNameValue(ModContent.ItemType<FebrezeHeavyDuty>())}";

					int hiveBackpackItemIndex = Main.LocalPlayer.FindItem(ItemID.HiveBackpack);
					var entitySource = NPC.GetSource_GiftOrReward();

					Main.LocalPlayer.inventory[hiveBackpackItemIndex].TurnToAir();
					Main.LocalPlayer.QuickSpawnItem(entitySource, ModContent.ItemType<FebrezeHeavyDuty>());
		...
		public override void OnGoToStatue(bool toKingStatue) {
			if (Main.netMode == NetmodeID.Server) {
				...
				packet.Write((byte)TheDollorama.MessageType.ExampleTeleportToStatue);
		...
		public override void TownNPCAttackProj(ref int projType, ref int attackDelay) {
			projType = ModContent.ProjectileType<Spray>();
			attackDelay = 1;
		}
