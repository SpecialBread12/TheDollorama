...
namespace TheDollorama.Content.NPCs
{
	...
	public class Employee : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			AIType = NPCID.GoblinScout; // Use vanilla zombie's type when executing AI code. (This also means it will try to despawn during daytime)
		...
		*/
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			// Vérifie si le joueur est dans le biome de la forêt et que c'est le jour
			if (spawnInfo.Player.ZoneForest && Main.dayTime)
			{
				return 0.2f; // Augmente la chance de spawn du NPC
			}

			return 0f; // Sinon, le NPC ne spawn pas
		}
		...
		public override void AI()
		{
			...
			{
				if (NPC.honeyWet)
				{ // Removes the effects of honey's fall rate making the NPC fall normally in honey
				...
				{ // Removes water falls speed effects, then adds honey falls speed effects, making the NPC fall at the honey rate in water
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("This simple employe is angry and will kill anyone to releave his stress (Need Texture)"),
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo)
		{
			...
			int buffType = ModContent.BuffType<Cuts>();
		...
		{
            if (modifiers.DamageType?.CountsAsClass(DamageClass.Magic) == true)
            {
            ...
            if (modifiers.DamageType?.CountsAsClass(ModContent.GetInstance<SprayDamage>()) == true)
            {
